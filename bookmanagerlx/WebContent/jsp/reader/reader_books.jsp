...
<div style="padding: 20px 550px 10px">
    <form method="post" action="${pageContext.request.contextPath}/reader" class="form-inline" id="searchform">
       <input type="hidden" name="methodName" value="searchBooks"/>
       <input type="hidden" name="readerId" value="${readercard.readerid}"/>
        ...
        <table class="table table-hover">
            ...
            <c:forEach items="${map.list}" var="book">
                <tr>
                    ...
                    <c:forEach var="lend" items="${map.myLendList}">
                        <c:if test="${lend eq book.bookid}">
                            <c:set var="flag" value="true"/>
                        </c:if>
                    </c:forEach>
                    <c:if test="${flag}">
                        <td><a href="${pageContext.request.contextPath}/reader?methodName=backBook&bookId=${book.bookid}&number=${book.number}">
                            <button type="button" class="btn btn-danger btn-xs">归还</button>
                        </a></td>
                    </c:if>
                    <c:if test="${not flag}">
                        <c:if test="${book.number>0}">
                            <td><a href="${pageContext.request.contextPath}/reader?methodName=lendBook&bookId=${book.bookid}&number=${book.number}">
                    ...
                    <td><a href="${pageContext.request.contextPath}/reader?methodName=bookDetail&bookId=<c:out value="${book.bookid}"></c:out>">
